...
export const simulate = (population, generation, callback) => {
    ...
    for (let i = 0; i < population.length; i++) {
        let offset = (i % 4) * 3;
        creatures.push(new Creature('something', population[i], world, getRandomColor(), offset));
    }
    ...
    const timer = setTimeout(() => {
        ...
        callback(population, creatures.map(c => c.maxDst));
    }, 6000);
};
